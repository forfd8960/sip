...
use crate::{
    astv1::{Assign, Binary, Literal, Node, NodeType, Program, VarStmt},
    errors::EvalError,
    object::Object,
    tokens::Token,
};
...
impl Interpreter {
    ...
    pub fn eval(&mut self, node: Box<dyn Node>) -> Result<Object, EvalError> {
        match node.typ() {
            NodeType::Null => Ok(Object::Null),
            NodeType::Literal => self.eval_literal(node.as_literal().unwrap()),
            NodeType::VarStmt => self.eval_var_stmt(node.as_var().unwrap()),
            NodeType::Assign => self.eval_assign(node.as_assign().unwrap()),
            NodeType::Binary => self.eval_binary(node.as_binary().unwrap()),
            _ => Err(EvalError::UnknowNode1(node.typ())),
        }
    }

    fn eval_literal(&self, literal: Literal) -> Result<Object, EvalError> {
        match literal.literal {
            ...
            _ => Err(EvalError::NotLiteral(literal.literal)),
        }
    }

    fn eval_var_stmt(&mut self, var_stmt: VarStmt) -> Result<Object, EvalError> {
        match var_stmt.name {
            Token::Ident(v) => {
                let value = Rc::try_unwrap(var_stmt.value);
                let final_val = self.eval(value.ok().unwrap())?;
                self.set_value(v, final_val.clone());
                Ok(final_val)
            }
            _ => Err(EvalError::NotIdent(var_stmt.name)),
        }
    }

    fn eval_assign(&mut self, assign: Assign) -> Result<Object, EvalError> {
        match assign.name {
            Token::Ident(ident) => {
                let value = Rc::try_unwrap(assign.value);
                let val = self.eval(value.ok().unwrap())?;
                self.set_value(ident, val.clone());
                Ok(val)
            }
            _ => Err(EvalError::TkIsNotIdent(assign.name)),
        }
    }

    fn eval_binary(&mut self, bin: Binary) -> Result<Object, EvalError> {
        let l = Rc::try_unwrap(bin.left);
        let r = Rc::try_unwrap(bin.right);

        let left_obj = self.eval(l.ok().unwrap())?;
        let right_obj = self.eval(r.ok().unwrap())?;
...
mod tests {
    use crate::{
        ast::Node, astv1, errors::EvalError, eval::Interpreter, eval1, object::Object,
        tokens::Token,
    };
    use std::rc::Rc;
    ...
    fn test_eval_integer() {
        let n = Box::new(astv1::Literal::new(Token::Integer(100)));
        let mut intpter = eval1::Interpreter::new();
    ...
    fn test_eval_literal1() {
        let n = Box::new(astv1::Literal::new(Token::SString(
